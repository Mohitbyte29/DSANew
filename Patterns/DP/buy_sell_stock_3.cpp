...
class Solution {
 int f(int i, int buy, vector<int>& prices, int n, int cap, vector<vector<vector<int>>>& dp){
        ...
        if(buy){
            dp[i][buy][cap] = max(-prices[i] + f(i + 1, 0, prices, n, cap, dp), 0 + f(i + 1, 1, prices, n, cap, dp));
        }
        else{
            dp[i][buy][cap] = max(prices[i] + f(i + 1, 1, prices, n, cap - 1, dp), 0 + f(i + 1, 0, prices, n, cap, dp));
        }
        return dp[i][buy][cap];
    }
public:
